...
def extract_questions_and_answers(text):
    ...
    pattern = re.compile(r"(Topic\s+1\s+Question\s+#\d+[\s\S]*?Correct Answer:\s*[A-Z]+(?:, [A-Z]+)*)", re.DOTALL)
    ...
    for match in matches:
        ...
        # 提取问题编号
        question_number = re.search(r'Topic\s+1\s+Question\s+#(\d+)', match).group(1)
...
def process_questions():
    with open('d:/lyon_study/quiz_check/quiz_check_fun/db/%s-c01-clean.txt' %kind, 'r', encoding='utf-8') as file:
        content = file.read()
...
with open('d:/lyon_study/quiz_check/quiz_check_fun/db/%s-c01.json' % kind,'w',encoding='utf-8') as f:
