...
def process_questions(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()
    pattern = re.compile(r"(Topic\s+12\s+Question\s+#\d+[\s\S]*?Correct Answer:\s*[A-Z]+)", re.DOTALL)
...
with open('d:/lyon_study/quiz_check/quiz_check_fun/db/%s-c01-clean-t12.txt' %kind,'w',encoding='utf-8') as f:
